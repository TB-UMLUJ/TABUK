...
const WebAuthnPromptModal: React.FC<WebAuthnPromptModalProps> = ({ isOpen, status, onClose }) => {
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-[60] flex justify-center items-center p-4" role="dialog" aria-modal="true">
            <div 
                className={`fixed inset-0 bg-black/70 backdrop-blur-sm ${isClosing ? 'animate-backdrop-out' : 'animate-backdrop-in'}`} 
                onClick={handleClose} 
            />
            
            <div 
                className={`relative bg-[#2A304C] rounded-2xl shadow-2xl w-full max-w-sm p-8 text-center text-white transform ${isClosing ? 'animate-modal-out-scale' : 'animate-modal-in-scale'}`}
            >
                 <p className="font-semibold text-lg mb-8 whitespace-nowrap">{currentMessage.title}</p>
                
                <div className="relative flex justify-center items-center mb-8 h-[7.5rem] w-full">
                   <StatusGraphic status={status} />
                </div>
                
                <p className="text-3xl font-bold tracking-wider">{currentMessage.progressValue}%</p>
                <p className="text-gray-300 mt-1 whitespace-nowrap">{currentMessage.progressText}</p>
            </div>
        </div>,
        modalRoot
    );
