...
const stepsConfig = [
    { id: 'basic', title: 'الأساسية', icon: <UserIcon className="w-5 h-5" /> },
    { id: 'job', title: 'الوظيفة', icon: <BriefcaseIcon className="w-5 h-5" /> },
    { id: 'certs', title: 'الشهادات', icon: <AcademicCapIcon className="w-5 h-5" /> },
    { id: 'contact', title: 'التواصل', icon: <ContactMailIcon className="w-5 h-5" /> },
    { id: 'docs', title: 'الملفات', icon: <DocumentTextIcon className="w-5 h-5" /> },
];
...
const AddEmployeeOnboarding: React.FC<AddEmployeeOnboardingProps> = ({ isOpen, onClose, onSave, employeeToEdit }) => {
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-50 flex justify-center items-center" role="dialog" aria-modal="true">
            ...
            <div className={`relative bg-gray-50 dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-2xl h-full sm:h-auto sm:max-h-[90vh] flex flex-col transform ${isClosing ? 'animate-modal-out' : 'animate-modal-in'}`}>
                                ...
                                {stepsConfig.map((step, index) => {
                                    ...
                                    return (
                                        <React.Fragment key={step.id}>
                                            <div 
                                                className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${isActive ? 'bg-primary text-white ring-4 ring-primary/20' : isCompleted ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400 dark:bg-gray-700'}`}
                                                title={step.title}
                                            >
                                                {isCompleted ? <CheckIcon className="w-4 h-4"/> : React.cloneElement(step.icon as React.ReactElement<any>, { className: 'w-4 h-4' })}
                                            </div>
                                            {index < stepsConfig.length - 1 && (
                                                <div className={`w-6 h-1 rounded-full transition-colors duration-300 ${isCompleted ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
                                            )}
                                        </React.Fragment>
                                    );
                                })}
                ...
                {/* Navigation Footer */}
                <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center bg-white dark:bg-gray-800 rounded-b-2xl flex-shrink-0">
                    <button onClick={handleBack} disabled={currentStep === 0 || isSaving} className="btn btn-secondary gap-2 disabled:opacity-50">
                        <ArrowRightIcon className="w-5 h-5" /> <span>رجوع</span>
                    </button>
                    {currentStep < stepsConfig.length - 1 ? (
                        <button onClick={handleNext} disabled={isSaving} className="btn btn-primary gap-2">
                            <span>التالي</span> <ArrowBackIcon className="w-5 h-5" />
                        </button>
                    ) : (
...
// --- Step Components with Card Styling ---
const StepWrapper: React.FC<{ title: string; icon?: React.ReactNode; children: React.ReactNode }> = ({ title, icon, children }) => (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-5 mb-2 border border-gray-200 dark:border-gray-700">
        <h3 className="font-bold text-lg text-gray-800 dark:text-white mb-4 flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 pb-3">
...
const BasicInfoStep: React.FC<{ data: typeof initialEmployeeState; updateData: Function }> = ({ data, updateData }) => {
    ...
    return (
        <StepWrapper title="المعلومات الأساسية" icon={<UserIcon className="w-6 h-6"/>}>
            ...
            <FormInput label="تاريخ الميلاد" name="date_of_birth" type="date" value={data.date_of_birth} onChange={(e) => updateData(e.target.name, e.target.value)} icon={<CalendarDaysIcon className="w-5 h-5" />} onFocus={handleFocus} onBlur={handleBlur} />
        </StepWrapper>
    );
};

const JobInfoStep: React.FC<{ data: typeof initialEmployeeState; updateData: Function }> = ({ data, updateData }) => (
    <StepWrapper title="البيانات الوظيفية" icon={<BriefcaseIcon className="w-6 h-6"/>}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormInput label="الرقم الوظيفي" name="employee_id" value={data.employee_id} onChange={(e) => updateData(e.target.name, e.target.value)} icon={<EmployeeIdIcon className="w-5 h-5" />} />
...
const CertificatesStep: React.FC<{ data: typeof initialEmployeeState; updateData: Function }> = ({ data, updateData }) => (
    <StepWrapper title="المؤهلات والشهادات" icon={<AcademicCapIcon className="w-6 h-6"/>}>
        <div className="space-y-4">
            {data.certificates.map((cert, index) => (
                <div key={cert.id} className="p-4 rounded-lg bg-gray-100 dark:bg-gray-700/50 border dark:border-gray-600">
                    ...
                    <button type="button" onClick={() => updateData('certificates', data.certificates.filter((_, i) => i !== index))} className="mt-2 text-danger text-sm font-semibold flex items-center"><TrashIcon className="w-4 h-4 ml-1"/>إزالة</button>
                </div>
            ))}
        </div>
...
const ContactInfoStep: React.FC<{ data: typeof initialEmployeeState; updateData: Function }> = ({ data, updateData }) => (
    <StepWrapper title="بيانات التواصل" icon={<ContactMailIcon className="w-6 h-6"/>}>
        ...
        <div className="space-y-4">
            {data.documents.map((doc, index) => (
                <div key={doc.id} className="p-4 rounded-lg bg-gray-100 dark:bg-gray-700/50 border dark:border-gray-600">
                    ...
                    <button type="button" onClick={() => updateData('documents', data.documents.filter((_, i) => i !== index))} className="mt-2 text-danger text-sm font-semibold flex items-center"><TrashIcon className="w-4 h-4 ml-1"/>إزالة</button>
                </div>
            ))}
        </div>
