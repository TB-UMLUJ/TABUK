...
const TaskCard: React.FC<TaskCardProps> = ({ task, onToggleComplete, onEdit, onDelete }) => {
    ...
    const formattedDate = task.dueDate 
        ? new Date(task.dueDate + 'T00:00:00.000Z').toLocaleDateString('ar-SA', { year: 'numeric', month: 'long', day: 'numeric' })
        : null;

    return (
        <div className={`bg-white rounded-xl shadow-md p-4 flex items-start gap-4 transition-all duration-300 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 ${task.isCompleted ? 'opacity-60' : ''}`}>
            <button
                ...
                aria-label={task.isCompleted ? "إلغاء إكمال المهمة" : "إكمال المهمة"}
            >
                <CheckIcon className="w-5 h-5" />
            </button>

            <div className="flex-1 min-w-0">
                <h3 className={`font-bold text-md text-gray-800 dark:text-white ${task.isCompleted ? 'line-through' : ''}`}>{task.title}</h3>
