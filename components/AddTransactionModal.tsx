...
const AddTransactionModal: React.FC<AddTransactionModalProps> = ({ isOpen, onClose, onSave, transactionToEdit, employees, officeContacts }) => {
    ...
    const handleSmartRouting = () => {
        ...
        if (employees.length > 0) {
            // Pick a random employee to simulate recommendation
            const recommendedEmployee = employees[Math.floor(Math.random() * employees.length)];
            setLinkType('employee');
            setTransactionData(prev => ({ ...prev, linked_employee_id: recommendedEmployee.id }));
            addToast(
                'تم تحديد الموظف الأنسب',
                `بناءً على التخصص وعبء العمل الحالي، نقترح: ${recommendedEmployee.full_name_ar}`,
                'info'
            );
        }
    };
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                         ...
                         <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                             <div className="flex justify-between items-center mb-2">
                                 <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">ربط المعاملة بـ (اختياري)</label>
                                 <button type="button" onClick={handleSmartRouting} className="text-xs flex items-center gap-1 text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-semibold transition-colors">
                                    <LightBulbIcon className="w-4 h-4" />
                                    اقتراح الموظف الأنسب
                                 </button>
                             </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
