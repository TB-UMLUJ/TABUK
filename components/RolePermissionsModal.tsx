...
const permissionNameMap: { [key: string]: string } = {
    ...
    'view_policies': 'عرض وتحميل السياسات', // Priority 1
    'manage_policies': 'إضافة سياسات',        // Priority 2
    'edit_policy': 'تعديل السياسات',          // Priority 3
    'delete_policy': 'حذف السياسات',          // Priority 4
    'manage_health_centers': 'إدارة المراكز الصحية',
...
const RolePermissionsModal: React.FC<RolePermissionsModalProps> = ({ isOpen, onClose, role, onSaveSuccess }) => {
    ...
    const groupedPermissions = useMemo(() => {
        ...
        allPermissions.forEach(p => {
            const name = p.permission_name.toLowerCase().trim();
            if (name.includes('user')) groups['المستخدمين والأدوار'].push(p);
            else if (name.includes('employee')) groups['الموظفين'].push(p);
            else if (name.includes('contact')) groups['تحويلات المكاتب'].push(p);
            else if (name.includes('task')) groups['المهام'].push(p);
            else if (name.includes('transaction')) groups['المعاملات'].push(p);
            else if (name.includes('policy') || name === 'view_policies') groups['الحوكمة والسياسات'].push(p);
            else if (name.includes('health_center') || name.includes('log')) groups['أخرى'].push(p);
            else groups['أخرى'].push(p); // Fallback for anything else
        });

        // Sort permissions in 'Governance' group for better UX: View -> Add -> Edit -> Delete
        const governanceOrder = ['view_policies', 'manage_policies', 'edit_policy', 'delete_policy'];
        groups['الحوكمة والسياسات'].sort((a, b) => {
            const indexA = governanceOrder.indexOf(a.permission_name.trim());
            const indexB = governanceOrder.indexOf(b.permission_name.trim());
            // Put known items first in defined order, unknown items at the end
                                        ...
                                        {permissions.map(p => (
                                            <label key={p.permission_id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                                ...
                                                <span className="font-medium text-gray-800 dark:text-gray-200">
                                                    {permissionNameMap[p.permission_name.trim()] || p.permission_name}
                                                </span>
                                            </label>
                                        ))}
                                    </div>
