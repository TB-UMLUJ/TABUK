...
import { Employee, OfficeContact, Task, Transaction, User, TransactionStatus, ImportSummary, ValidationIssue, UpdatePreview, UpdateSelection, Certificate, EmployeeDocument, HealthCenter } from './types';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        if (sessionStorage.getItem('appIsUpdating') === 'true') {
            sessionStorage.removeItem('appIsUpdating');
            addToast('اكتمل التحديث', 'تم تحديث التطبيق بنجاح!', 'success');
        }
    }, [addToast]);


    // --- جلب البيانات والتعبئة الأولية من Supabase ---
    useEffect(() => {
        const fetchDataAndSeed = async () => {
            ...
            try {
                ...
                // Check and Seed Employees (Added)
                const { count: employeesCount, error: employeesCountError } = await supabase.from('employees').select('*', { count: 'exact', head: true });
                if (employeesCountError) throw employeesCountError;

                if (employeesCount === 0) {
                    const employeesToInsert = mockEmployees.map(({ id, ...rest }) => rest);
                ...
                const [
                    ...
                    { data: healthCentersData, error: healthCentersError },
                ] = await Promise.all([
                    ...
                    supabase.from('health_centers').select('*, manager:employees(id, full_name_ar)').order('name', { ascending: true }),
                ]);
                ...
                if (healthCentersError) throw healthCentersError;

                setEmployees(employeesData || []);
                setOfficeContacts(contactsData || []);
                setTasks(tasksData || []);
                setTransactions(transactionsData || []);
                setHealthCenters(healthCentersData as HealthCenter[] || []);
    ...
    useEffect(() => {
        ...
        const employeesSubscription = supabase.channel('employees-realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'employees' }, (payload) => {
                ...
                if (payload.eventType === 'DELETE') {
                    const oldEmployeeRef = payload.old as { id: number };
                    setEmployees(prevEmployees => {
                        const employeeToDelete = prevEmployees.find(e => e.id === oldEmployeeRef.id);
                        addToast(`حذف: ${employeeToDelete?.full_name_ar || 'سجل'}`, '', 'deleted');
                        return prevEmployees.filter(e => e.id !== oldEmployeeRef.id);
                    });
                }
            }).subscribe();
        ...
        const contactsSubscription = supabase.channel('office_contacts-realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'office_contacts' }, (payload) => {
                ...
                if (payload.eventType === 'DELETE') {
                    const oldContactRef = payload.old as { id: number };
                    setOfficeContacts(prevContacts => {
                        const contactToDelete = prevContacts.find(c => c.id === oldContactRef.id);
                        addToast(`حذف: ${contactToDelete?.name || 'سجل'}`, '', 'deleted');
                        return prevContacts.filter(c => c.id !== oldContactRef.id);
                    });
                }
            }).subscribe();
        ...
        const tasksSubscription = supabase.channel('tasks-realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'tasks' }, (payload) => {
                ...
                if (payload.eventType === 'DELETE') {
                    const oldTaskRef = payload.old as { id: number };
                    setTasks(prevTasks => {
                        const taskToDelete = prevTasks.find(t => t.id === oldTaskRef.id);
                        addToast(`حذف مهمة: ${taskToDelete?.title || ''}`, '', 'deleted');
                        return prevTasks.filter(t => t.id !== oldTaskRef.id);
                    });
                }
            }).subscribe();
        ...
        const transactionsSubscription = supabase.channel('transactions-realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'transactions' }, (payload) => {
                ...
                if (payload.eventType === 'DELETE') {
                    const oldTransactionRef = payload.old as { id: number };
                    setTransactions(prevTransactions => {
                        const transactionToDelete = prevTransactions.find(t => t.id === oldTransactionRef.id);
                        addToast(`حذف معاملة: ${transactionToDelete?.subject || ''}`, '', 'deleted');
                        return prevTransactions.filter(t => t.id !== oldTransactionRef.id);
                    });
                }
            }).subscribe();
            
        const healthCentersSubscription = supabase.channel('health_centers-realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'health_centers' }, async (payload) => {
                const enrichCenter = async (center: HealthCenter): Promise<HealthCenter> => {
                    if (!center.manager_employee_id) {
                        return { ...center, manager: null };
                    }
                    const { data: managerData, error } = await supabase
                        .from('employees')
                        .select('id, full_name_ar')
                        .eq('id', center.manager_employee_id)
                        .single();
                    if (error) {
                        console.error('Failed to fetch manager for health center', error);
                        return { ...center, manager: null };
                    }
                    return { ...center, manager: managerData };
                };

                if (payload.eventType === 'INSERT') {
                    const newCenter = await enrichCenter(payload.new as HealthCenter);
                    setHealthCenters(prev => [...prev, newCenter].sort((a,b) => a.name.localeCompare(b.name, 'ar')));
                    addToast(`تمت إضافة مركز: ${newCenter.name}`, '', 'success');
                }
                if (payload.eventType === 'UPDATE') {
                    const updatedCenter = await enrichCenter(payload.new as HealthCenter);
                    setHealthCenters(prev => prev.map(c => c.id === updatedCenter.id ? updatedCenter : c).sort((a,b) => a.name.localeCompare(b.name, 'ar')));
                    addToast(`تم تحديث مركز: ${updatedCenter.name}`, '', 'info');
                }
                if (payload.eventType === 'DELETE') {
                    const oldCenterRef = payload.old as { id: number };
                    setHealthCenters(prevCenters => {
                        const centerToDelete = prevCenters.find(c => c.id === oldCenterRef.id);
                        addToast(`تم حذف مركز: ${centerToDelete?.name || ''}`, '', 'deleted');
                        return prevCenters.filter(c => c.id !== oldCenterRef.id);
                    });
                }
            }).subscribe();
        ...
        return () => {
            ...
            supabase.removeChannel(healthCentersSubscription);
        };
    ...
    const handleSaveHealthCenter = async (centerData: Omit<HealthCenter, 'id' | 'manager'> & { id?: number }) => {
        const dataWithTimestamp = { ...centerData, updated_at: new Date().toISOString() };
        const { error } = await supabase.from('health_centers').upsert(dataWithTimestamp);
        
        if (error) {
            addToast('فشل حفظ المركز الصحي', `حدث خطأ: ${error.message}`, 'error');
            throw error; // إعادة رمي الخطأ لإبقاء النافذة المنبثقة مفتوحة
        }
    };

    const handleDeleteHealthCenter = async (center: HealthCenter) => {
        requestConfirmation('تأكيد الحذف', `هل أنت متأكد من رغبتك في حذف مركز "${center.name}"؟`, async () => {
            const { error } = await supabase.from('health_centers').delete().eq('id', center.id);
            if (error) {
                addToast('فشل الحذف', `حدث خطأ: ${error.message}`, 'error');
            }
        });
